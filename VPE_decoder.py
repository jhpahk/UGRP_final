...
from model_using_decoder import EncoderEstimator
...
class VideoPoseEstimator(nn.Module):
    ...
    def forward(self, img):
        if self.count % self.N == 0:
            ...
            self.prev_keymaps = self.model_encoder.encoder(img)
            self.prev_keypoints = joints
            self.count += 1
            return joints
            ...
            if self.count % self.N == 1:
                self.model_encoder.set_memory(self.prev_keymaps, self.prev_keypoints)
            self.count += 1

...
class EncoderEstimator(nn.Module):
    def __init__(self, pretrained="autoencoder/UGRP_AutoEncoder/checkpoint/checkpoint_2021_10_09_04:42:23/checkpoint_2021_10_09_04:42:23_epoch_70.pth", key_size = 3):
        super().__init__()
        ...
        self.key_size = key_size  # should be odd number
    ...
    def get_keys(self, frame, annot):        # frame image(256x256)와 annotation(256x256)을 주면, 관절에 해당하는 key vectors를 뽑아준다.
        ...
        if self.key_size != 1:      # 만약 key size가 1이 아닐 경우 
            keymap = pad(keymap, [self.key_size - 1] * 4)
        ...
        for i in range(len(annot)):
            if annot[i] == 0:
                ...
                keys.append(torch.zeros(1, 32, self.key_size, self.key_size))
            else:
                # keys.append(crop(keymap, annot[i][1], annot[i][0], 1, 1).cuda())
                keys.append(crop(keymap, annot[i][1] , annot[i][0] , self.key_size, self.key_size))
    ...
    def calc_distance(self, target, target_point, query, weighted=True):
        ...
        # distance_test = torch.norm((query - target), dim=1)

        query = pad(query, [self.key_size - 1] * 4)

        distance = []
        for i in range(64):
            distance_row = []
            for j in range(64):
                target_area = crop(query, i, j, self.key_size, self.key_size)
                distance_row.append(torch.norm((target_area - target)))

            distance_row = torch.tensor(distance_row, dtype=torch.float, requires_grad=True)
            distance.append(distance_row)    
        distance = torch.unsqueeze(torch.stack(distance).cuda(), 0)

        # print("distance's shape: {}".format(distance.shape))
    ...
    def forward(self, query):
        ...
        # if torch.all(self.memory_keys[i] == torch.zeros(1, 32, 1, 1).cuda()):
            if torch.all(self.memory_keys[i] == torch.zeros(1, 32, self.key_size, self.key_size)):
                joints.append(None)
                continue
            ...
            self.memory_keys[i] = crop(pad(keymap_query, [self.key_size - 1] * 4), min_vertical, min_horizontal, self.key_size, self.key_size)     # 해당 joint에 대한 memory를 query key 값으로 대체한다.
